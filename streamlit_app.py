...
def main():
    ...
    # Ввод стороннего — только для режима 1
    if mode_val == 1:
        st.markdown("---")
    ...
    # Ввод объёмов — только для режима 3
    if mode_val == 3:
        st.markdown("---")
    ...
    # Целевой As, k и выход — для всех режимов
    st.markdown("---")
    ...
    if st.button("Рассчитать"):
        ...
        # Для режима 3 принудительно As_target=0 (необязательно)
        if mode_val == 3:
            As_target = 0.0

        # Вызов расчёта с учётом Q_base/Q_ext только для 3
        ...
        # Отображение таблицы результатов без изменений
