...
def main():
    ...
    # Ввод стороннего концентрата для режима 1
    if mode_val == 1:
        st.markdown("---")
    ...
    # Целевые параметры для всех режимов
    st.markdown("---")
    ...
    if st.button("Рассчитать"):
        # Автоподсчёт Seq, если оставлено 0
        ...
        # Для режима 3 принудительно игнорируем As_target
        if mode_val == 3:
            As_target = 0.0

        # Вызываем расчёт, передаём Q_base и Q_ext во все режимы
        ...
        # Формируем и показываем таблицу результатов
